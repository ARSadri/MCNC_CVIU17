function [H_T, R_T, Likli] = MSNC_funda(X, model_type, G, n_samples, labels)
[~,sind]=sort(labels);
...
%     disp(labels(inds))
    psub = X(:,inds);
    iter_cnt = 0;
    k_start_space_end = max([1 k_m-psize]);
    k_start_space = floor(linspace(1, k_start_space_end, n_FLKOS_itrs + 1));
    while( iter_cnt < n_FLKOS_itrs )
        theta = feval(fitfn,psub);
        dist_old = feval(resfn,theta,X);
        [~,I_old] = sort(dist_old);
        k_start = k_start_space(iter_cnt + 1); 
        linspaced_k_p_to_k = floor(linspace(k_start, k_m, psize));
        inds = I_old( linspaced_k_p_to_k);
%         disp(labels(inds))
        psub = X(:,inds);
        iter_cnt = iter_cnt+1;
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        k_start_space_end = max([1 postBasedK-psize_tmp]);
        k_start_space = floor(linspace(1, k_start_space_end, n_FLKOS_itrs + 1));
        for  iter_cnt = 1 : n_FLKOS_itrs
            k_start = k_start_space(iter_cnt + 1);
            linspaced_k_p_to_k = floor(linspace(k_start, postBasedK, psize_tmp));
            inds = I( linspaced_k_p_to_k );
            psub = X(:,inds);
            theta = feval(fitfn,psub);
            dist = feval(resfn,theta,X);
            [~,I] = sort(dist);
...
%         y_phi(:,1:end-1) = y_from_Q0(:,1:sz) / PHI_R;
        ...
        theta = y_phi;
        iter_cnt = 0;
        k_start_space_end = max([1 postBasedK_old-psize_tmp_old]);
        k_start_space = floor(linspace(1, k_start_space_end, n_FLKOS_itrs + 1));
        while( iter_cnt < n_FLKOS_itrs )
            dist_y_F = feval(resfn,theta,X);
            [~,I] = sort(dist_y_F);
            k_start = k_start_space(iter_cnt + 1); 
	        linspaced_k_p_to_k = floor(linspace(k_start, postBasedK_old, psize_tmp_old));
            inds = I( linspaced_k_p_to_k );
            psub = X(:,inds);
            iter_cnt = iter_cnt+1;
